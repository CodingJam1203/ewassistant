...
> **운영 결정 (2026-05-18)** — ew-assistant(Nclick)는 **STG 환경을 영구 미사용**한다 (인프라 크레딧 절약). 표준 배포 흐름:
> 🔨 개발 진행중 → 🚀 DEV_배포완료 → 🔍 DEV_QA진행중 → ✔ DEV_QA완료 → 🎉 PROD_배포완료 (dev → main fast-forward 직진)
> §A·§C의 STG 관련 상태/전이(🚫 표식)는 옵션에만 남아 실 사용 안 함. **stg 브랜치는 삭제 X — 방치**. §E "STG 단계 생략"은 사용자 명시 없이 **default**. 단 §E 필수 가드(PROD DB 마이그레이션 사전 점검 등)는 그대로 살아있음.
...
| ✔ DEV_QA완료 | DEV QA 통과 → PROD 배포 트리거 대기 | 사용자 |
| 🚀 STG_배포완료 | 🚫 운영 미사용 (2026-05-18 결정) | — |
| 🔍 STG_QA진행중 | 🚫 운영 미사용 (2026-05-18 결정) | — |
| ✔ STG_QA완료 | 🚫 운영 미사용 (2026-05-18 결정) | — |
| 🎉 PROD_배포완료 | 사용자 트리거로 Claude가 PROD 배포 완료 (dev → main fast-forward) | Claude (배포 후) |
...
| **✔ DEV_QA완료 → 🎉 PROD_배포완료** | **수동 트리거 → 자동 실행** | **사용자 트리거 시 Claude가 dev → main fast-forward push** (STG 단계 영구 스킵) |
| ✔ DEV_QA완료 → 🚀 STG_배포완료 | 🚫 운영 미사용 | (2026-05-18 결정) |
| 🚀 STG_배포완료 → 🔍 STG_QA진행중 | 🚫 운영 미사용 | (2026-05-18 결정) |
| 🔍 STG_QA진행중 → ✔ STG_QA완료 | 🚫 운영 미사용 | (2026-05-18 결정) |
| 🔍 STG_QA진행중 → 🚧 개발 요청 (재작업) | 🚫 운영 미사용 | (2026-05-18 결정) |
| ✔ STG_QA완료 → 🎉 PROD_배포완료 | 🚫 운영 미사용 | (2026-05-18 결정) |
...
### E. 단축 — STG 영구 스킵이 default + 기타 사용자 명시 단축

> **2026-05-18 운영 결정** — 첫 번째 단축(STG 단계 영구 생략)은 ew-assistant의 **default 배포 흐름**이 되었다. 사용자가 별도 명시("바로 prod로", "main까지" 등) 하지 않아도 dev → main 직진. stg 브랜치는 방치(merge 안 함). 단축 박제 코멘트는 더 이상 commit message에 추가하지 않음 (default라 noise).
...
1. **PROD DB 변경(마이그레이션·RLS·스키마)** 이 포함되면 적용 전 **사전 점검 SQL 결과를 사용자에게 보고**하고 응답을 받는다. STG가 스킵되는 default 흐름에서는 더더욱 중요 — DEV에서만 검증되고 바로 PROD로 가므로.
2. 비밀값(`.env`, secrets) 마스킹, 자동 롤백 금지, 강제 push·hook skip 금지 등 기본 보안 가드는 그대로 적용.
3. DB 마이그레이션 점검 + PROD 배포는 사용자 트리거가 있은 후 진행. dev push까지는 작업 흐름 따라 자동.
