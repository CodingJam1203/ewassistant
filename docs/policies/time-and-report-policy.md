...
| **시간 기준 today/prior 자동 분기 (v1.44)** | **당일 + 현재 KST < `planned_start_time` → today (출근예정·퇴근예정·메모·장소만, 실출근 input hide). 당일 + 현재 ≥ planned 또는 과거 일자 → prior (실출근·퇴근예정·메모·장소, 출근예정 hide).** 모든 팀 통일 — use_check_in_complete=true 팀의 "수정 클릭 시 자동 출근완료" 부작용 차단 + false 팀 planned 이후 실출근 정정 경로 통합. | `CheckInModal.tsx` mode 결정 + 실출근 div는 `caseMode==='prior'`에만 노출 + submit `safeActualCheckIn`이 today/future면 `''` 강제 | ✅ |
