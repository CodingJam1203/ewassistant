...
**주말 정기 알림 스킵 (v1.39)** — 정기 cron 알림(`morning-summary` 07시 / `reminder-20` 20시 / `reminder-22` 22시)은 **대상 출근일이 토·일요일**이고 해당 팀에 **출근보고 작성자가 0명**이면 그 팀 알림을 **발송하지 않는다** (전원 미보고인 주말 알림은 노이즈라 스킵). 판정 — `isWeekendDate(대상일)` && 팀 내 출근보고(`work_logs` 작성) 0명. 대상일: morning-summary=오늘, reminder-20/22=내일. "출근보고 작성자"만 카운트(휴가/반차만 있고 출근보고 0인 팀도 스킵). 평일 대상 알림은 종전대로 전원 미보고여도 발송.
...
|---|---|---|---|---|
| 출근예정 | "미보고" 잠금 (placeholder) | 토글 풀어야 입력 가능. **단 `use_check_in_complete=true` 팀은 출근예정 영역 전체 숨김** — 항상 미보고(`planned_start_time=NULL`), 필요 시 [출근보고 수정]에서 입력 (v1.36) | `CheckInModal.tsx` (`hideExpectedStart`) | ✅ |
...
| 8.1.7 미보고 첫 출근 모달 — 출근예정 숨김 (v1.36) | `use_check_in_complete=true` 팀의 **미보고 첫 출근(`caseMode=none`)** 모달도 출근예정 영역 숨김(§5.1) → 항상 미보고/NULL. [출근보고 수정](`today`)·[사전 출근보고](`future`)는 현행대로 출근예정 노출. | `CheckInModal.tsx` (`hideExpectedStart`) | ✅ |
...
✅ **D5. 캘린더 "미보고" 칩 렌더 (2026-05-18 v1.6 · 2026-05-22 v1.41 근무일 07시 경계)**
...
- **근무일 경계 = 07시 (v1.41, 2026-05-22)** — "오늘/어제" 판정의 기준일을 달력 자정이 아니라 **근무일(07시 경계)** 로 둔다. 새벽 00:00~07:00은 **전날 근무의 연장**으로 보아, 그 전날 row가 07시 전까지는 미보고/퇴근누락으로 잡히지 않는다. (새벽 퇴근자가 자정 직후 곧장 "퇴근누락"으로 분류되던 문제 해소)
  - **구현** — `getKstWorkDateString()` (`src/lib/utils/date.ts`, `WORK_DAY_BOUNDARY_HOUR=7`). `missed-checkout`·`missing-reports`·`submission-status` 세 라우트의 "오늘" 기준을 이 함수로 통일.
  - **적용 범위 한정** — 출근(체크인)·팀 현황 보드의 "오늘"에는 적용하지 않는다 (06시 조기출근을 전날로 오기입하는 부작용 방지). 미보고 게이트 + 퇴근보고 폼 "당일" 판정에만 적용.
- **예외** — 오늘이라도 출/퇴근 모두 작성된 경우(조기 퇴근)는 `complete`로 인정.
- **구현** — `submission-status/route.ts` (오늘 미보고는 `'future'`로 분류), `missing-reports/route.ts` (`effectiveTo = yesterday`). 두 곳 모두 기준일 = `getKstWorkDateString()`.
