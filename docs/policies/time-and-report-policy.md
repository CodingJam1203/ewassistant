...
| **구글캘린더 연동뷰** | `/calendar` (상단 Navbar "일정관리" 탭, "제출 내역" 다음) | 전 구성원 (로그인) | org_calendar_events 캐시를 사용자×날짜 매트릭스로. Google 캘린더와 **실시간 양방향 동기화**. 일정 등록/수정 가능(본인 본부 한정, admin 전체). | `src/app/calendar/page.tsx` |
...
- 구글캘린더 연동뷰는 상단 Navbar의 "일정관리" 탭(`/calendar`, "제출 내역" 다음)으로 일반 구성원이 진입한다 (`Navbar.tsx` navLinks).
